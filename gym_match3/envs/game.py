...
class Board(AbstractBoard):
    ...
    def __init__(self, rows, columns, n_shapes, immovable_move, immovable_shape=-1):
        self.__rows = rows
        self.__columns = columns
        self.__n_shapes = n_shapes
        self.__immovable_shape = immovable_shape
        self.__immovable_move = immovable_move
    ...
    def shuffle(self, random_state=None):
        if self.__immovable_move:
            moveable_mask = self.board <= self.__n_shapes
        else:
    ...
    def new(self, random_state=None):
        moveable_mask = self.board != self.immovable_shape
        board_ravel = np.random.randint(0, self.__n_shapes, np.sum(moveable_mask))
        self.put_mask(moveable_mask, board_ravel)
...
class Filler(AbstractFiller):
    ...
    def __init__(self,n_of_match_counts_immov, immovable_move,
                 match_counts_add_immovable, number_of_match_counts_add_immovable,
                 step_add_immovable, number_of_step_add_immovable,
                 random_state=None):
        self.__n_of_match_counts_immov = n_of_match_counts_immov
        self.__num_of_immovable = 0
        self.__immovable_move = immovable_move
        
        self.__match_counts_add_immovable = match_counts_add_immovable
        self.__number_of_match_counts_add_immovable = number_of_match_counts_add_immovable
        self.__match_count = 0
        
        self.__step_add_immovable = step_add_immovable
        self.__number_of_step_add_immovable = number_of_step_add_immovable
        self.__step = 0
        
        self.__random_state = random_state
    
    def step(self):
        if self.__step_add_immovable:
            self.__step += 1
    ...
    def move_and_fill(self, board: Board, fill=True, start=False):
        self.__move_nans(board)
        if fill:
            self.__fill(board, start)
    ...
    def __fill(self, board, start=False):
        is_nan_mask = np.isnan(board.board)
        num_of_nans = is_nan_mask.sum()
        
        if self.__match_counts_add_immovable:
            self.__match_count += num_of_nans
            self.__num_of_immovable += self.__n_of_match_counts_immov * (self.__match_count // self.__number_of_match_counts_add_immovable)
            self.__match_count %= self.__number_of_match_counts_add_immovable
            
        if self.__step_add_immovable:
            self.__num_of_immovable += self.__n_of_match_counts_immov * (self.__step // self.__number_of_step_add_immovable)
            self.__step %= self.__number_of_step_add_immovable  
        
        if self.__num_of_immovable > 0 and not start:
            if self.__num_of_immovable >= num_of_nans:
                new_shapes = np.array([board.immovable_shape] * num_of_nans)
                self.__num_of_immovable -= num_of_nans
            else:
                immovable = np.array([board.immovable_shape] * self.__num_of_immovable)
                random_shape = np.random.randint(low=0, high=board.n_shapes, size=(num_of_nans - self.__num_of_immovable))
                new_shapes = np.array([*immovable, *random_shape])
...
class Game(AbstractGame):
    ...
    def __init__(self, rows, columns, n_shapes, length, immovable_move,
                 n_of_match_counts_immov,
                 match_counts_add_immovable, number_of_match_counts_add_immovable,
                 step_add_immovable, number_of_step_add_immovable,
                 no_legal_actions_do, 
                 all_moves=False, immovable_shape=-1, random_state=None):
        ...
        self.__filler = Filler(n_of_match_counts_immov=n_of_match_counts_immov,
                               immovable_move=immovable_move,
        ...
        self.__no_legal_actions_do = no_legal_actions_do
    ...
    def start(self, board: np.ndarray or None or Board):
        ...
        if board is None:
            ...
            board = RandomBoard(rows, cols, self.board.n_shapes, self.__immovable_move)
            ...
            self.board = board
        self.__operate_until_possible_moves(start=True)
        self.__filler.reset()
        return self

    def __start_random(self):
        rows, cols = self.board.board_size
        tmp_board = RandomBoard(rows, cols, self.board.n_shapes, self.__immovable_move)
    ...
    def simulation_swap(self, point: Point, point2: Point):
        direction = point2 - point
        score = 0
        tmp_board = self.__get_copy_of_board()
        tmp_filler = copy.deepcopy(self.__filler)
        tmp_board.move(point, direction)
        matches = self.__mtch_searcher.scan_board_for_matches(tmp_board)
        while len(matches) > 0:
            score += len(matches)
            tmp_board.delete(matches)
            tmp_filler.move_and_fill(tmp_board, False)
            matches = self.__mtch_searcher.scan_board_for_matches(tmp_board)
        return score
    ...
    def __operate_until_possible_moves(self, start=False):
        """
        scan board, then delete matches, move nans, fill
        repeat until no matches and appear possible moves
        """
        score = self.__scan_del_mvnans_fill_until(start=start)
        self.__shuffle_until_possible(start=start)
        return score

    def __get_matches(self):
        return self.__mtch_searcher.scan_board_for_matches(self.board)
    
    def get_all_possible_moves(self, all_moves=True):
    ...
    def __scan_del_mvnans_fill_until(self, start=False):
        score = 0
        matches = self.__get_matches()
        score += len(matches)
        while len(matches) > 0:
            self.board.delete(matches)
            self.__filler.move_and_fill(self.board, start=start)
            matches = self.__get_matches()
            score += len(matches)
        return score

    def __shuffle_until_possible(self, start=False):
        possible_moves = self.__get_possible_moves()
        while len(possible_moves) == 0:
            if start:
                self.start(None)
            elif self.__no_legal_actions_do == "shuffle":
                self.board.shuffle(self.__random_state)
            elif self.__no_legal_actions_do == "new":
                self.board.new(self.__random_state)
            elif self.__no_legal_actions_do == "terminal":
                break
            self.__scan_del_mvnans_fill_until(start=start)
            possible_moves = self.__get_possible_moves()
        return self
